...
Breve descrição
----------------
Este projeto é baseado em TanStack Start e usa Vite como bundler/dev server. É um template para aplicações React modernas com integração TanStack (Router, Query) e Tailwind CSS.
...
- `pnpm lint` / `pnpm format` / `pnpm check` — ferramentas de lint/format (Biome neste projeto).

Notas sobre o framework e ferramentas
-----------------------------------
Vite oferece um servidor de desenvolvimento extremamente rápido com HMR; TanStack Start fornece convenções e integrações para construção de stores, rotas e dados (React + TanStack). Tailwind CSS é usado para estilização.
